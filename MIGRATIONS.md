...
| 024 | `dispute_evidence_video.sql` | **NOT YET APPLIED** | Adds `disputes.evidence_video text`. The dispute form already uploads the buyer's video walkthrough to the `dispute-evidence` bucket (required for claims over $500) and the admin queue already renders a link for it, but the column never existed so the URL was dropped on submit. Code degrades gracefully if unapplied: the dispute is still recorded (without the video link) and an ops alert fires so the video can be attached by hand. Apply to stop losing video evidence. |
| 025 | `admin_audit_log.sql` | **NOT YET APPLIED** | Adds the `admin_audit_log` table (append-only; admins-read RLS, service-role writes) recording privileged admin actions — dispute resolution, artwork approve/reject, feature toggle — with actor, target, and detail. `logAdminAction()` degrades gracefully if unapplied (a missing table is logged and swallowed, never blocks the admin action). Apply to start capturing the trail. |

...
|---|---|---|---|---|
| [`/api/cron/release-escrow`](src/app/api/cron/release-escrow/route.ts) | `0 * * * *` (hourly) | 48h inspection window ([orders/[id]/deliver/route.ts:54](src/app/api/orders/[id]/deliver/route.ts:54)); 14d shipped→delivered backstop ([escrow.ts `SHIPPED_AUTODELIVER_DAYS`](src/lib/stripe/escrow.ts)) | Two passes per fire: (1) auto-deliver orders shipped >14d ago with no buyer confirmation, so an artist is paid even when the buyer never clicks "confirm delivery"; (2) release funds past the inspection deadline. Auto-deliver sets a fresh 48h window, so a just-auto-delivered order is not released in the same run. Hourly caps delivery-to-payout at ~49h. | 2026-04-24 (natural fire 00:00:04 UTC, 200); auto-deliver pass added B3 2026-07-29 |

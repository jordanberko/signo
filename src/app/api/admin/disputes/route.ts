...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = serviceClient
      .from('disputes')
      .select(
        '*, orders(id, total_amount_aud, dispatch_photo_urls, artworks(title), buyer:profiles!orders_buyer_id_fkey(full_name, email), artist:profiles!orders_artist_id_fkey(full_name, email, street_address, city, postcode, country))',
      )
